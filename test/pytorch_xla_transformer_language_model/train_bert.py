...
def loop_bert_with_amp(model, input, labels, optimizer, xla_enabled, autocast, scaler):
  with autocast():
...
def main_bert(index):
  ...
  def train_loop_fn(loader):
    ...
    for iteration, batch in enumerate(loader):
      ...
      if amp_enabled:
        loss = loop_bert_with_amp(model, input, target, optimizer, xla_enabled, autocast, scaler)
      else:
...
if __name__ == '__main__':
  ...
  if xla_enabled:
    ...
    from test.pytorch_xla_transformer_language_model.modeling_bert import BertForSequenceClassification

  xmp.spawn(main_bert, args=())
